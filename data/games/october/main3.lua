...
me().description = function(s)
	if here() ^ 'park1' then
		p [[Кажется, со мной все в порядке.]]
		return
	end
...
function game:before_Draw(w)
	...
	if have 'plakat' then
		...
		if w:find("^на ") or w:find("по ") or w:find("^под ") then
...
cutscene {
	...
	text = {
		...
		Это всё не важно, потому что я нашел тебе работу!!! Одевайся, через час у тебя
		собеседование!^
		-- Собеседование?]],
		...
		-- Молчи! Не хочу слушать этот бред! Я знаю заранее всё то, что ты мне сейчас скажешь! Лучше послушай меня! Хочешь ты этого или нет, тебе придется играть по правилам игры! Мир крутится деньгами! Если не понимаешь этого, тебя выбросит за борт! Так что...]],
		[[-- Извини, Борис, я не могу сейчас говорить. Я должен дорисовать картину, потом созвонимся...^
		-- Да пошел ты к чертям собачьим!^^
...
obj {
	...
	before_Attack = [[Потом придется покупать новый.]];
...
Furniture {
	...
	description = [[Мои картины плохо продаются, но на жизнь, все-таки, хватает.]];
...
cutscene {
	...
	text = {
		...
		[[Когда я достигну края? Мир завертелся у меня перед глазами. Внезапно я отчетливо почувствовал запах мокрой листвы.]];
	};
	next_to = 'park1';
}
...
room {
	...
	enter = function(s, f)
		if f ^ 'fall' then
			...
			что цел, поднялся на ноги. Это был парк. Это не могло быть правдой, но я было так!]]
			if not have'brush' then
	...
	dsc = [[Я нахожусь в парке. Мокрые осенние листья под ногами. Неподалеку я вижу скамейку.]];
...
Distance {
	...
	before_Talk = [[Я произнес про себя молитву.]];
}
...
room {
	...
	title = "У озера";
	dsc = function(s)
		p [[По поверхности озера идёт легкая рябь. Красно-желтые
	листья, опавшие с деревьев растущих вдоль берега, плавают в тёмной воде.
...
room {
	...
	dsc = function(s)
		...
		p [[Неподалеку я вижу подземный переход рядом с которым установлен столб.
...
room {
	...
	enter = function(s, f)
		set_pic 'underground';
		if f^'street' then
			p [[Я подошел к переходу и спустился вниз.]]
	...
	obj = {
		...
		door {
			...
			description = function(s)
				...
				if not alarm then
					p [[Я подошел к железной решётчатой двери.
...
room {
	...
	before_Default = function(s, ev, w)
		...
		p [[Все, что я могу, я уже и так делаю.]];
	end;
...
obj {
	...
	before_Talk = function(s)
			if not s.talked and not _'truba'.seen then
				...
				p [[-- А вообще, попробуйте позвать кого-нибудь на помощь -- произнес я в надежде, что
...
obj {
	...
	before_Talk = [[-- Эй, блоховоз!!! -- неуверенно крикнул я псу. Он на короткое время отвлекся от дерева, но потом принялся за старое. Может быть, к лучшему?]];
	before_Attack = function(s, w)
		if w^'stone' then
...
obj {
	...
	before_Talk = function(s)
		p [[-- Кис, кис, кис, кис!!!^^Котенок не прореагировал.]]
