...
cutscene {
	...
	text = {
		...
		-- Молчи! Не хочу слушать этот бред! Я знаю заранее всё, что ты мне сейчас скажешь! Лучше послушай меня! Хочешь ты этого или нет, тебе придётся играть по правилам игры! Мир крутится деньгами! Если не понимаешь этого, тебя выбросит за борт! Так что...]],
	...
	before_Talk = function(s)
		...
		if here()^'sad' then
			...
			if not s.panic then
				...
				-- Ан пюедо! Дехано сен Аба! -- мальчик вырвал руку и отбежал от меня на безопасное расстояние.]]
				return
			else
...
room {
	...
	dsc = function(s)
		...
		p [[Ветви деревьев молчаливо смотрят в хмурое небо. Главная дорожка парка, идущая с севера на юг,
		огибает небольшое озеро и заканчивается воротами главного входа.]];
	end;
...
obj {
	...
	["before_Attack,Push"] = function(s, w)
		...
		if not have 'plakat' then
			move('plakat', 'entrance')
			_'plakat':attr'~moved'
			_'plakat'.seen = false
		end
		walkin 'street'
...
cutscene {
	...
	text = {
		...
		Самолёты! Их гул был уже отчётливо слышен, даже невзирая
	...
	description = function(s)
		...
		if not have(s) then
			if s:hasnt'moved' then
				if here() ^ 'underground2' then
					p [[Большой, немного помятый лист бумаги лежит на полу.]];
				else
					p [[Большой, немного помятый лист бумаги лежит на земле.]];
				end
			else
		...
		p [[Я расправил лист бумаги. На нём был изображён плакат.^^
			...
			Над ними была проведена красная черта.]]
		if here() ^ 'underground2' or here() ^ 'underground' then
			p [[Над чертой чёрной краской были изображены птицы...
			Крупные птицы заполонили всю верхнюю часть плаката. Или...^^
			Самолёты! И бомбы! Они сбрасывали бомбы, которые взрывались о красную
			черту -- мать и ребёнок были в безопасности.]]
			p [[Я всё понял! Это же бомбоубежище!]]
		else
			p [[Над чертой чёрной краской были изображены самолёты, сбрасывающие бомбы...]]
		end
		p [[^^Обратная сторона плаката совершенно чистая.]];
